...
# **************************************** G ****************************************
def 类_获取属性或方法(类名称, 属性或方法名称, 异常返回=None):
    """
    【函数简介】
      - 从指定类对象中获取指定属性或方法。
    【函数原型】
...
def Unicode_转为整数(字符):
    """
    【函数简介】
      将传入的字符转换为它对应的ASCII或Unicode编码的整数。用于获取字符的整数编码,与 Unicode_转为字符() 互为反向函数。
    【函数原型】
...
def 类_方法转属性(获取属性=None, 设置属性=None, 删除属性=None, 属性描述=None):
    """
    【函数简介】
...
def 类_设置属性或方法(类名称, 属性或方法名称, 属性或方法定义):
    """
    【函数简介】
      - 用于为类对象设置属性。
...
def 转为字符串(需转换对象='', 编码格式='utf-8', 异常处理='strict'):
    ...
    :return: 字符串
    """
    打印(isinstance(需转换对象, bytes))
    if isinstance(需转换对象, bytes) is True:
